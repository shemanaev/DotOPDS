...
public class LuceneIndexStorage : IDisposable
{
    ...
    protected List<Book> Search(Query query, int take, int skip, out int count)
    {
        ...
        using var directory = new SimpleFSDirectory(new DirectoryInfo(_indexPath));
    ...
    public void Open()
    {
        directory = new SimpleFSDirectory(new DirectoryInfo(_indexPath));
    ...
    private static Document MapBook(Book book)
    {
        ...
        document.Add(new Field("pubdate", DateTools.DateToString(book.Date, DateTools.Resolution.DAY), Field.Store.YES, Field.Index.NOT_ANALYZED));
    ...
    {
        "ru" => new RussianAnalyzer(Version.LUCENE_30),
        _ => new StandardAnalyzer(Version.LUCENE_30),
    };
}
